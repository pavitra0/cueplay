...
//   const tmdbId = await (tmdbResults[0]?.id)
...
export default async function VideoPlayerPage({ params }) {
  ...
  let shortData, posterData, type, formatedEpisodes = null;

  try {
    const movieData = await getMovie(id);
    shortData = movieData.shortData;
    posterData = movieData.posterData;
    type = shortData?.["@type"];

    if (type === "TVSeries") {
      const episodesData = await getEpisodes(shortData.alternateName || shortData.name);
      formatedEpisodes = episodesData?._embedded?.episodes
        ? organizeEpisodes(episodesData._embedded.episodes)
        : { seasons: [] };
    }
  ...
  return (
    <VideoPlayerClient
      ...
      episode={safeEpisode}
      poster={poster}
      formatedEpisodes={formatedEpisodes}
    />
  );
}
